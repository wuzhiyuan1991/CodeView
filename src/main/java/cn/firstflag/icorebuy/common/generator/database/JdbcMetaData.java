...
public class JdbcMetaData {
    ...
    public static void getDataBaseInfo() {  
    	Connection conn =  JdbcConnection.getConnection(false);
    	ResultSet rs = null;
        try{  
     ...
     * @throws
     */
    public static List<TableInfo> getTablesList() {
    	List<TableInfo> tList = new ArrayList<TableInfo>();
    	Connection conn = JdbcConnection.getConnection(true);
    	ResultSet rs = null;
        try { 
            ...
            while (rs.next()) {
                ...
                remarks = null ==remarks?"":remarks.replace("\r", " ").replace("\n", " ");
     ...
     * @throws
     */
    public static TableInfo getTablesInfo(String table_Name){
    	Connection conn =  JdbcConnection.getConnection(true);
	    ResultSet rs = null;
	    try {
	      //设置连接属性,使得可获取到表的REMARK(备注)
	       ...
	       */
	      if(null != table_Name && table_Name.trim().length()>0){
//	    	  table_Name = table_Name.trim().toUpperCase();
	    	  table_Name = table_Name.trim().toLowerCase();
	  	  }
	      
	      rs = dbmd.getTables(null, null, table_Name, new String[]{"TABLE","VIEW"}); 
     ...
     * @throws
     */
    public static List<TindexInfo> getPrimaryKeysInfo(String table_Name) {
    	
    	List<TindexInfo> tindexs = new ArrayList<TindexInfo>();
        ...
        try{  
        	 ...
        	 */
            rs = dbmd.getPrimaryKeys(null, null, null == table_Name?null:table_Name.trim().toUpperCase());  
            ...
            while (rs.next()){  
                ...
                TindexInfo ti = new TindexInfo();
                ti.setColumnName(columnName);
                ti.setIndexname(pkName);
                ti.setKeySeq(keySeq);
                tindexs.add(ti);
            }  
        }catch (SQLException e){  
     ...
     * @throws
     */
    public static List<TindexInfo> getImportedKeysInfo(String table_Name) {
        ...
        try{  
        	...
        	/**
        	 * 获取对给定表的主键列的描述
        	 * 方法原型:ResultSet getImportedKeys(String catalog,String schema,String table);
        	 * catalog - 表所在的类别名称;""表示获取没有类别的列,null表示获取所有类别的列。
        	 * schema - 表所在的模式名称(oracle中对应于Tablespace);""表示获取没有模式的列,null标识获取所有模式的列; 可包含单字符通配符("_"),或多字符通配符("%");
        	 * table - 表名称;可包含单字符通配符("_"),或多字符通配符("%");
        	 */
            rs = dbmd.getImportedKeys(null, null, null == table_Name?null:table_Name.trim().toUpperCase());  
            ...
            while (rs.next()){  
            	...
            	String columnName = rs.getString("COLUMN_NAME");//列名  
                short keySeq = rs.getShort("KEY_SEQ");//序列号(主键内值1表示第一列的主键，值2代表主键内的第二列)  
                String fkName = rs.getString("FKCOLUMN_NAME"); //外键名称    
                
                TindexInfo ti = new TindexInfo();
                ti.setColumnName(columnName);
                ti.setIndexname(fkName);
                ti.setKeySeq(keySeq);
                tindexs.add(ti);   
            }  
        }catch (SQLException e){  
     ...
     * @throws
     */
    public static List<TcolumnInfo> getColumnsInfo(String table_Name,String column_Name){
    	List<TcolumnInfo> tcs = new ArrayList<TcolumnInfo>();
    	Connection conn =  JdbcConnection.getConnection(false);
	    ResultSet rs = null;
	    ...
	    try{
	      ...
	      if(null != column_Name && column_Name.trim().length()>0){
	    	  column_Name = column_Name.trim().toUpperCase();
	  	  }
	      rs =dbmd.getColumns(null, null, table_Name, column_Name);
	      ...
	      while(rs.next()){
	        ...
	        TcolumnInfo tc = new TcolumnInfo();
	        tc.setColumnName(columnName);
	        tc.setColumnType(dataTypeName);
	        tc.setColumnLength(columnSize);
	        tc.setDecimalDigits(decimalDigits);
	        tc.setDefaultValue(columnDef);
	        tc.setIsNullAble(1 == nullAble ? true : false);
	        remarks = null ==remarks?"":remarks.replace("\r", " ").replace("\n", " ");
     ...
     * @throws
     */
    public static void getIndexInfo(String table_Name) { 
    	Connection conn =  JdbcConnection.getConnection(false);
    	ResultSet rs = null;
        try{  
        	DatabaseMetaData dbmd = conn.getMetaData();
        	 ...
        	 */
            rs = dbmd.getIndexInfo(null, null, null == table_Name?null:table_Name.trim().toUpperCase(), false, true);  
            while (rs.next()){  
            	String tableCat = rs.getString("TABLE_CAT");  //表类别(可为null) 
            	String tableSchemaName = rs.getString("TABLE_SCHEM");//表模式（可能为空）,在oracle中获取的是命名空间,其它数据库未知     
            	String tableName = rs.getString("TABLE_NAME");  //表名  
  ...
  public static void main(String[] args) {
    ...
    getTablesInfo(null);  //获取表信息
    getPrimaryKeysInfo("tbl_user"); //获取表主键信息
    getImportedKeysInfo("tbl_area");
    getIndexInfo("tbl_user");  //获取表索引信息
    getColumnsInfo(null,null); //获取表中列值信息
  }
}
